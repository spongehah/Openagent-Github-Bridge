...
Present the fix to the user in the issue thread through `skill github-progress-comment` and wait for explicit approval before proceeding.
...
- Report the result back to the issue through `github-progress-comment`:
  > ✅ PR created successfully: {PR_URL}
  > Please review the PR page for any CI checks or reviewer feedback.
- Reuse the same mutable progress comment if one was created earlier in the task. Do not add an extra wrap-up comment when updating that comment is enough.
...
- **GitHub handoff:** Use `github-progress-comment` as the default way to publish implementation status, approval wait-state, and final PR result back to the issue thread unless that skill is unavailable.
