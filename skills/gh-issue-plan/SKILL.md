---
name: gh-issue-plan
description: 通过 issue 号用 gh 读取 GitHub issue，结合当前仓库代码做需求分析，并把实现计划回写到同一个 issue 的进度评论中。凡是用户提到“根据 issue 做方案”“分析 issue 需求”“按 issue 号结合代码给计划”“输出计划内容/补充/疑问”这类场景，都应使用这个 skill。
---

# gh-issue-plan

当用户给出 issue 号，希望你结合当前仓库代码产出一份可执行的实现方案，并把方案写回 GitHub issue 时，使用这个 skill。

这个 skill 的核心约定：

1. 先解析目标 GitHub 仓库，再用 `gh` 读取 issue。
2. 在做深入分析前，先提醒自己调用 `github-progress-comment`，确保 GitHub 线程中尽早出现进度评论。
3. 在给出计划前，必须先检查代码仓库，而不是只看 issue 文本。
4. 可以积极使用 subagent 并行探索，但最终方案和 GitHub 评论更新必须由主 agent 收口。
5. 最终把计划内容写回同一条进度评论，而不是再额外发一条总结评论。
...
最终产出是写回 GitHub issue 进度评论的一段计划内容。评论生命周期由 `github-progress-comment` 负责；这个 skill 负责产出和更新其中的计划内容块。

计划内容块里必须保留稳定 marker，便于后续技能或流程识别。
...
在大规模读代码或启动 subagent 之前，先调用 `github-progress-comment` 创建或复用进度评论。

这个 skill 不再单独定义“如何创建临时评论、如何更新最终评论”的命令流程，那部分由 `github-progress-comment` 统一负责。

这里关心的是：当你更新那条进度评论时，要把本 skill 的计划内容块写进去，并保留本 skill 的 marker，便于后续 `gh-pr-create` 识别。

如果仓库 prompt 对进度评论的固定 sections 有要求，先遵守仓库 prompt，再把下面的内容块嵌入到对应位置。
...
这里说的“同一条评论”，指的是 `github-progress-comment` 管理的那条进度评论，不是 issue 作者最初正文，也不是别人发的第一条评论。
...
不要让 `github-progress-comment` 管理的进度评论永远停留在“进行中”状态。

如果被阻塞，也要更新同一条进度评论中的本 skill 内容块，例如：
