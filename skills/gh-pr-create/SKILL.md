---
name: gh-pr-create
description: 通过 issue 号用 gh 读取 GitHub issue 与已有方案评论，结合当前仓库完成代码实现、创建 PR，并把修改总结和 PR 链接回写到同一个 issue 的进度评论中。凡是用户提到“按 issue 实现”“根据方案写代码并提 PR”“读取 issue comment 里的 plan 后落地代码”“关联 issue 和 PR”这类场景，都应使用这个 skill。
---

# gh-pr-create

当用户希望从 GitHub issue 出发，按已有计划完成编码、创建 PR，并把结果写回 issue 时，使用这个 skill。

这个 skill 的核心约定：

1. 先解析目标 GitHub 仓库，再用 `gh` 读取 issue。
2. 读取 issue 下已经存在的 plan 评论。
3. 在开始改代码前，先提醒自己调用 `github-progress-comment`，确保 GitHub 线程中尽早出现进度评论。
4. 在当前仓库完成代码实现。
5. 用 `gh` 创建 PR。
6. 把修改总结、验证结果和 PR 链接写回同一条进度评论。
...
第一个 marker 用于识别计划评论，第二个 marker 用于标记当前 skill 写回到进度评论中的结果内容块。
...
在深入分析或开始改代码前，先调用 `github-progress-comment` 创建或复用进度评论。

这个 skill 不再单独定义“如何创建临时评论、如何更新最终评论”的命令协议，那部分由 `github-progress-comment` 统一负责。

这个 skill 关心的是：在那条进度评论里，回填一个带有 `<!-- skill:gh-pr-create -->` marker 的结果内容块，方便后续识别和人工查看。

如果仓库 prompt 已经规定了进度评论的固定 sections，先遵守仓库 prompt，再把下面的内容块嵌入对应位置。
...
这条 issue 评论内容块要简洁。详细设计理由、实现细节和完整验证上下文应主要放在 PR 中。
...
如果编码或 PR 创建失败，也必须更新同一条进度评论中的本 skill 内容块，例如：
